...
5. **历史走势纳入**（`JQ_USE_TREND_IN_SELECT`）：批量拉个股日线，用**均线结构 / 中期斜率 / 相对 MA20 位置**算“历史走势健康度”，计入综合分；明显**跌破 60 日线的下行趋势**可直接否决（`JQ_TREND_VETO_BELOW_MA60`）；
6. **综合评分**：`主力净占比 + 连续净流入 + 涨跌幅健康度 + 换手健康度 + 历史走势健康度` 加权（`JQ_SCORE_WEIGHTS`），按综合分排序取前 N；
7. 输出候选股（可建仓或交给 Claude 分析）。
...
> `JQ_USE_TREND_IN_SELECT`、`JQ_TREND_LOOKBACK`、`JQ_TREND_VETO_BELOW_MA60`、
> `JQ_USE_MARKET_REGIME`、`JQ_MARKET_REGIME_INDEX`、`JQ_WEAK_MARKET_*`。
...
| **基本面** fundamental | 12% | 营收增速(连续 3 季加速)、毛利率变化(改善/转正)、净利润+经营性现金流(盈利且现金流为正更抗跌)；短线交易中基本面滞后，权重下调 |
| **筹码结构** chips | 20% | 换手衰减法近似筹码成本分布 → 获利盘比例 / 主力成本区 / 集中度；A 股筹码信号价值高，权重上调 |
...
新增 `shizixi_strategy.py`，直接对接用户给出的批量 K 线接口：

```bash
python3 shizixi_strategy.py --codes 688188 --since 2020-12-29 --to 2026-08-07 --period daily --adjust qfq
python3 shizixi_strategy.py --codes 688188,688981,300750 --since 2020-12-29 --to 2026-08-07
```

策略参数（股票代码、起止时间、周期、复权方式、limit、接口地址）均可通过命令行配置。模型用“均线多头排列 + 放量突破 N 日新高 + 20 日正动量”生成买入信号，用“跌破中期均线 / 均线转弱 / ATR 止损止盈 / 最大持有天数”生成卖出信号。回测会输出每只股票的交易次数、胜率、总收益、年化收益、最大回撤、夏普、盈亏比，并在多只股票输入时按综合表现排序，标出胜率和收益率更高的股票。

收盘后运行时，程序会根据最后一个交易日的信号给出下一交易日买卖建议：出现 `BUY` 时给出参考买入价、ATR 止损和目标价；无信号时提示等待条件满足；已有持仓信号则提示持有/卖出触发条件。

> 注意：历史回测只验证规则在样本期内的表现，不保证未来收益；实盘前请继续做样本外验证和模拟盘验证。
